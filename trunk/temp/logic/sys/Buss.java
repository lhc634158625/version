...
public class Buss extends CommonBuss{
	...
	public Result<UserInfo> SaveUser(UserInfo userInfo) throws NoSuchAlgorithmException, UnsupportedEncodingException {
		...
		String _id = (String)dbHelp.Save(userInfo, "UserInfo", (userInfo.getId() == 0));
		if(userInfo.getId() == 0) {
			
		}
		
		int id = Integer.parseInt(_id);
		userInfo = (UserInfo) dbHelp.GetById(id, "UserInfo");
   ...
   public Result<List<Station>> GetSuperiorsById(Request request) {
	      DBHelp dbHelp=new DBHelp();
	      List<Station> Station= (List<Station>)dbHelp.GetMultiByRequest(request,"GetSuperiorsById");
	      return new Result<List<Station>>(Station);  
	     }
   
   public Result<List<Station>> GetStationTree(Request request) {
	      DBHelp dbHelp=new DBHelp();
