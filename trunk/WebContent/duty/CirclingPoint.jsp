							...
							<form class="layui-form" action="">
								<div class="layui-inline">
									<div class="layui-input-inline" style="margin-top: 5px;">
										<input type="checkbox" name="like1" lay-skin="primary" id="encrypt" lay-filter="checkitem"
											title="<img src='../images/gray.png'><img src='../images/blue.png'>&nbsp;围村围点">
									</div>
								</div>
			...
			.use(
					...
					function() {
						...
						form.on('checkbox(checkitem)', function(obj){    
							//当前元素    
							var data = $(obj.elem)[0];    
							if(data.id=="hasArrage" || data.id=="noArrage"){
								var showPointType=0;
								var showHasNo=0;
								var check = document.getElementById("hasArrage").checked;
								if(check){
								...
								map.ReShowMarkByType("circling",showPointType,showHasNo);
							}							
							if(data.id=="staffOnline" || data.id=="staffOutline"){
								var showPointType=0;
								var showHasNo=0;
								var check = document.getElementById("staffOnline").checked;
								if(check){
									showHasNo = showHasNo | 2;
								}
								...
								map.ReShowMarkByType("circlingStaff",showPointType,showHasNo);
							}
...
//	var whole_organizeStation = document.getElementById("organizeStation");// 关联单位
	...
	//获取icon状态(11二进制，每一位分别表示一个状态，顺序依次是在线，离线)
	function getShowTypebyParam(state){
		var showType= 0;
		if(state=="在线"){
			showType = showType | 2;
		}else{
	...
	function getOrganizeStation(fromTime,toTime,pointInfoId){
		var organizeStation=new Array();
		var strConditions = "pointInfoId,int,in," + pointInfoId+";workDt,string,>=,"+fromTime+";workDt,string,<=,"+toTime;
		PostData("point/pointInfoStation/filter",
				createRequest(0, 1000, "id", strConditions),
				function(result) {						
					if(result.data.length>0){
						organizeStation=result.data;
					}
				},false);		
		return organizeStation;
	}
	function delOrganizeStation(fromTime,toTime,pointInfoId){
		var idslist=new Array();
		var strConditions = "pointInfoId,int,=," + pointInfoId+";fromTime,string,=,"+fromTime+";toTime,string,=,"+toTime;
		PostData("point/pointInfoStation/filter",
				createRequest(0, 1000, "id", strConditions),
				function(result) {
					...
					if(itemList.length>0){
						for(var i=0;i<itemList.length;i++){
							idslist.push(itemList[i].id);
						}						
					}
				},false);
		if(idslist.length>0){
			PostData("point/pointInfoStation/deleteList",idslist,function(result) {
			},false);
		}
	}
	function delOrganizeStationbyDate(fromDate,toDate,pointInfoId){
		var idslist=new Array();
		var strConditions = "pointInfoId,int,in," + pointInfoId+";workdt,string,>=,"+fromDate+";workdt,string,<=,"+toDate;
		PostData("point/pointInfoStation/filter",
				createRequest(0, 1000, "id", strConditions),
				function(result) {
					var itemList = result.data;		
	...
	function saveOrganizeStation(organizeStation){
		var flag=true;
		if(organizeStation!=null && organizeStation.length>0){
			PostData("point/pointInfoStation/saveList",organizeStation,function(result) {
				if(result.code==0){
					flag=true;
				}
				else{
...
<script type="text/javascript">
				...
				if(arrangeInfos!=null &&arrangeInfos.length>0){
					...
					if(flag==0 || (flag==1 && cover==true)){
						if(flag==1 && cover==true){//覆盖，先删除
							...
							delOrganizeStationbyDate(toDate,toDate,strPointId);
						}
						for(var i=0; i<arrangeInfos.length;i++){
						...
						var orgStations=getOrganizeStation(fromDate,fromDate,strPointId);
						for(var i=0; i<orgStations.length;i++){
							orgStations[i].workDt=toDate;
							orgStations[i].fromTime=toDate+ orgStations[i].fromTime.substr(10,8);
							orgStations[i].toTime=toDate+ orgStations[i].toTime.substr(10,8);							
							delete orgStations[i].id;
						}
						//保存排班和排班设备信息
						var saveArrangeFlag=saveArrangeInfos(arrangeInfos);
						var saveDeviceFlag=saveDeviceInfos(deviceInfos);
						var saveOrgFlag=saveOrganizeStation(orgStations);
						if(saveArrangeFlag==true && saveDeviceFlag==true && saveOrgFlag==true){
							ShowTip("温馨提示","复制排班成功!");
							return true;
						}
					}
	...
	function CreateHeadHtml(type,title,strDate,isEdit) {
		...
		if(isEdit == 0){
			if(type==1){
				html += "<td colspan='14'>各分局重点防务巡逻安排表("+strDate+")</td>";
			}else if(type==2||type==3){
				html += "<td colspan='14'>"+title+"重点防务巡逻安排表("+strDate+")</td>";
			}
		}else{
			if(type==1){
				html += "<td colspan='13'>各分局重点防务巡逻安排表("+strDate+")</td>";
			}else if(type==2||type==3){
				html += "<td colspan='13'>"+title+"重点防务巡逻安排表("+strDate+")</td>";
			}
		}
		html += "</tr>"
		html += "<tr>";
		html += "<td rowspan='2'>分局(队伍驻地)</td>";
		html += "<td rowspan='2'>关联单位</td>";		
		html += "<td rowspan='2'>目标区域</td>";	
		html += "<td rowspan='2'>行动时间</td>";		
		html += "<td colspan='3'>带队民警</td>";
		html += "<td rowspan='2'>组织单位</td>";
		html += "<td colspan='3'>出动警力</td>";		
	...
	function CreateBodyHtml(itemId,type,fromTime,toTime,isEdit,pageId){
		...
		if(pointInfos!=null && pointInfos.length>0){
			...
			var organizeStations= getOrganizeStation(fromTime,toTime,strPointId);	
			...
			var html= "";
			if(isEdit!=0){
				html += createTableBody(listRowIdx,pointInfos,arrangeInfos,deviceInfos,whole_subStationMap,arrangeTimeSpan,pointInfoDeviceDict,getOrganizeNameMap(organizeStations));
			}else{
				html += createTableEditBody(listRowIdx,pointInfos,arrangeInfos,deviceInfos,whole_subStationMap,arrangeTimeSpan,pointInfoDeviceDict,pageId,getOrganizeNameIdMap(organizeStations))
			}
			return html;
		}
		return "";
	}
	function getOrganizeNameMap(orgList){
		var tempMap=new Map();
		for(var i=0;i<orgList.length;i++){
			var key=orgList[i].pointInfoId+","+new Date(orgList[i].fromTime).Format("yyyy-MM-dd hh:mm")
			+","+new Date(orgList[i].toTime).Format("yyyy-MM-dd hh:mm")+","+new Date(orgList[i].workDt).Format("yyyy-MM-dd");
			if(tempMap.has(key)){
				tempMap.set(key,(tempMap.get(key)+","+orgList[i].stationName));
			}else{
				tempMap.set(key,orgList[i].stationName);
			}
		}
		return tempMap;
	}
	function getOrganizeNameIdMap(orgList){
		var tempMap=new Map();
		for(var i=0;i<orgList.length;i++){
			var key=orgList[i].pointInfoId+","+new Date(orgList[i].fromTime).Format("yyyy-MM-dd hh:mm")
			+","+new Date(orgList[i].toTime).Format("yyyy-MM-dd hh:mm")+","+new Date(orgList[i].workDt).Format("yyyy-MM-dd");
			var value=orgList[i].stationId+","+orgList[i].stationName +","+orgList[i].id;
			if(tempMap.has(key)){
				tempMap.set(key,(tempMap.get(key)+";"+value));
			}else{
	...
	function getPointInfoDeviceDict(){
		...
		for(var i=0;i<pointInfoDeviceDict.length;i++){
			...
			if(typeof tempDict.code !="undefined" && (tempDict.name=="民警"||tempDict.name=="武警"||tempDict.name=="协警"||tempDict.name=="简要战果"||tempDict.name=="备注")){
	...
	//扽到表格数据
	//substation 分局ID序列
	function createTableBody(listRowIdx,pointInfos,arrangeInfos,deviceInfos,substation,arrangeTimeSpan,pointInfoDeviceDict,orgMap){
		var html="";		
		html += "<tbody>";
		for(var i=0;i<listRowIdx.length;i++){
			...
			if(spanRows>0){
				for(var j=0;j<spanRows;j++){				
					...
					var orgKey=itemPoint.id+","+fromTime+","+toTime+","+workDt;
					var orgstation="";
					if(orgMap.has(orgKey)){
						orgstation=orgMap.get(orgKey);
					}
					...
					html += "<td rowspan='"+rows +"'>"+orgstation+"</td>";	
	...
	//扽到表格编制数据
	//substation 分局ID序列
	function createTableEditBody(listRowIdx,pointInfos,arrangeInfos,deviceInfos,substation,arrangeTimeSpan,pointInfoDeviceDict,pageId,orgIdMap){
		var html="";	
			...
			if(spanRows>0){
				for(var j=0;j<spanRows;j++){				
					...
					if(orgIdMap.has(orgKey)){
						orgValue=orgIdMap.get(orgKey).split(";");
					}
					html += "<tr>";				
					html += "<td id='areaId_"+pageId+"_"+itemPoint.pid+"'>"+substation.get(itemPoint.pid)+"</td>";		
					html += "<td id='type_"+pageId+"_"+itemPoint.pid+"'>" + itemPoint.stationName+ "</td>";
					...
					var stationinfo="";					
					for(var k=0;k<orgValue.length;k++){	
						stationinfo +="<div id='stationinfo_"+pageId+"_"+itemPoint.id+"_"+orgValue[k].split(",")[2]+"' style='width: 100%;background-color: #92B4F4;margin-bottom: 5px;'>";
						stationinfo += orgValue[k].split(",")[0]+","+orgValue[k].split(",")[1];							
						stationinfo += "<span style='display: inline-block; color:white;float: right;margin-right: 5px;cursor: pointer;' onclick='delStation(this)'>x</span></div>";
					}					
					html += "<td id='tdorgStation_"+pageId+"_"+itemPoint.id+"_"+timeSpanIdx+"' onclick='showTreemMean(this)'>" + stationinfo+ "</td>";
				...
				html += "<td id='tdorgStation_"+pageId+"_"+itemPoint.id+"_"+timeSpanIdx+"' onclick='showTreemMean(this)'></td>";
				var strDeviceTypeCode = "";
				for(var m=0;m<pointInfoDeviceDict.length;m++){
	...
	function delStaff(obj) {
		...
		var inputId="";
		if(item[4]==1){
			inputId=$(obj).parent().parent().next().next().children().attr("id");
		}else{
	...
	function createAddTr(temp,pageId,itemPoint) {
		...
		html += "<td id='stdorgStation_"+pageId+"_"+itemPoint.id+"' onclick='showTreemMean(this)'></td>";
	...
	function modifyStaffAndDeviceMess(id,pageId,timeSpanIdx,strDeviceTypeCode,type,identify) {
		var shiftType=whole_shiftTypeIdMap.get("围村围点班");
		...
		var divsOrg = document.getElementById("tdorgStation_"+pageId+"_"+id+"_"+timeSpanIdx).getElementsByTagName("div");
		var orgStationList = new Array();
		for(var i=0;i<divsOrg.length;i++){
			var orgStation=new Object();			
			var item = divsOrg[i].innerText;
			var temp=item.substr(0,item.length-1).split(",");
			orgStation.stationId=temp[0];
			orgStation.stationName=temp[1];
			orgStation.pointInfoId=id;
			orgStation.workDt = searchBegin;
			orgStation.fromTime = fromTime;
			orgStation.toTime = toTime;
			orgStationList.push(orgStation);
		}		
		delOrganizeStation(fromTime,toTime,id);
		if(orgStationList.length>0){
			saveOrganizeStation(orgStationList);
		}
	...
	function delStaffAndDeviceMess(pointInfoId,fromTime,toTime,pageId) {
		...
		delOrganizeStation(fromTime,toTime,pointInfoId);
		makeOrder(pageId);
	}
	...
	function saveShiftAndDeviceMess(temp,pageId,pointId,stationId) {
		...
		var eLongGun = document.getElementById("eLongGun_"+pageId+"_"+pointId).value;// 简要战果
		var eHandGun = document.getElementById("eHandGun_"+pageId+"_"+pointId).value;// 备注
		var devicesInfoMap = new Map();
		var devicesInfoArr = ["执勤人数,民警;"+nPeople+"","执勤人数,武警;"+nArmed +"","执勤人数,协警;"+nAuxiliary
							+"","点位装备,简要战果;"+eLongGun+"","点位装备,备注;"+eHandGun+""];
		...
		var divsOrg = document.getElementById("stdorgStation_"+pageId+"_"+pointId).getElementsByTagName("div");
		var orgStationList = new Array();
		for(var i=0;i<divsOrg.length;i++){
			var orgStation=new Object();
			var item = divsOrg[i].innerText;
			...
			orgStation.stationId=temp[0];
			orgStation.stationName=temp[1];
			orgStation.pointInfoId=pointId;
			orgStation.workDt = searchBegin;
			orgStation.fromTime = fromTime;
			orgStation.toTime = toTime;
			orgStationList.push(orgStation);
		}		
		delOrganizeStation(fromTime,toTime,pointId);
		if(orgStationList.length>0){
			saveOrganizeStation(orgStationList);
		}
		makeOrder(pageId);
...
var whole_treeResultId="";
	function showTreemMean(obj) {
		...
		else if(treeSelectId.substr(0,14)=="stdorgStation_" || treeSelectId.substr(0,13)=="tdorgStation_"){
			SelectorItem.Initial(true,"tree",1,0,1);
		}
		else{
	...
	function changeMultiStation(nodes){
		var stationIdMap=new Map();
		var divsOrg = document.getElementById(whole_treeResultId).getElementsByTagName("div");
		var tdContent=document.getElementById(whole_treeResultId);
		var tdContentVal = tdContent.innerHTML;
		var orgStationList = new Array();
		for(var i=0;i<divsOrg.length;i++){	
			var item = divsOrg[i].innerText;
			...
			var temp=item.split(",");
			stationIdMap.set(temp[0],temp[1]);
		}
		var tdid=tdContent.id;
		var pageId=tdContent.id.split("_")[1];
		var pointInfoId=tdContent.id.split("_")[2];
		var  names = "";
      	for (var i = 0; i < nodes.length; i++) {
      		if (nodes[i].check_Child_State == 1)
          		continue;
      		if(stationIdMap.has(nodes[i].id+""))
      			continue;
      		
     		tdContentVal +="<div style='width: 100%;background-color: #92B4F4;margin-bottom: 5px;'>";
     		tdContentVal +=nodes[i].id+","+nodes[i].name;							
     		tdContentVal += "<span style='display: inline-block; color:white;float: right;margin-right: 5px;cursor: pointer;' onclick='delStation(this)'>x</span></div>";  	 
	  	}
      	tdContent.innerHTML=tdContentVal;
	}
</script>
<script>
