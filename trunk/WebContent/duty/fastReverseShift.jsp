...
<body class="layui-layout-body">
							...
							<form class="layui-form" action="">
								<div class="layui-inline">
									<div class="layui-input-inline" style="margin-top: 5px;">
										<input type="checkbox" name="like1" lay-skin="primary" id="oneMin" lay-filter="checkitem"
											title="<img src='../images/gray1.png'><img src='../images/blue1.png'>&nbsp;一分钟 ">
										<input type="checkbox" name="like1" lay-skin="primary" id="threeMin" lay-filter="checkitem"
											title="<img src='../images/gray3.png'><img src='../images/blue3.png'>&nbsp;三分钟">
										<input type="checkbox" name="like1" lay-skin="primary" id="fiveMin" lay-filter="checkitem"
									...
									<div class="layui-input-inline">
										<ul>
											<li class="liStyle">勤务车辆
												<ul class="ulStyle">
													<li><input type="checkbox" name="like1" id="carOnline" lay-filter="checkitem"
														lay-skin="primary" title="在线 "></li>
													<li><input type="checkbox" name="like1" id="carOutline" lay-filter="checkitem"
	...
	<div id="carOptionDiv" style="position: absolute;display: none; border: solid 1px #ccc; background-color: #fff;width: 350px;z-index: 1;">
		<div style="width: 300px;margin: 5px auto;">
			<select id="carOptionMess"  multiple="" xm-select="carOptionStaff" xm-select-search="" xm-select-create="" xm-select-height="36px">
			</select>
		</div>
		<div style="float: right;padding-bottom: 5px;margin-right: 25px;">
			<button	class="layui-btn layui-btn-primary" id="carDeviceEnsure">确定</button>
		</div>
	</div>
</body>
			...
			.use(
					...
					function() {
						...
						form.on('checkbox(checkitem)', function(obj){    
							//当前元素    
							var data = $(obj.elem)[0];    
							if(data.id=="oneMin" || data.id=="threeMin" || data.id=="fiveMin" || data.id=="hasArrage" || data.id=="noArrage"){
								var showPointType=0;
								var showHasNo=0;
								var check = document.getElementById("oneMin").checked;
								if(check){
									showPointType = showPointType | 16;
								}
								check = document.getElementById("threeMin").checked;
								if(check){
									showPointType = showPointType | 8;
								}
								check = document.getElementById("fiveMin").checked;
								if(check){
								...
								map.ReShowMarkByType("135",showPointType,showHasNo);
							}
							if(data.id=="oneMin" || data.id=="threeMin" || data.id=="fiveMin" || data.id=="carOnline" || data.id=="carOutline"){
								var showPointType=0;
								var showHasNo=0;
								var check = document.getElementById("oneMin").checked;
								if(check){
								...
								check = document.getElementById("carOnline").checked;
								if(check){
									showHasNo = showHasNo | 2;
								}
								check = document.getElementById("carOutline").checked;
								if(check){
									showHasNo = showHasNo | 1;
								}	
								map.ReShowMarkByType("135car",showPointType,showHasNo);
							} 
							if(data.id=="oneMin" || data.id=="threeMin" || data.id=="fiveMin" || data.id=="staffOnline" || data.id=="staffOutline"){
								var showPointType=0;
								var showHasNo=0;
								var check = document.getElementById("oneMin").checked;
								if(check){
								...
								map.ReShowMarkByType("135staff",showPointType,showHasNo);
							}
...
<script type="text/javascript">
	...
	function getAllPointInfo(dateOne,dateTwo){
		...
		//135所有的辖区
		var pointTypes = whole_pointInfoMap.get("1分钟") + "," + whole_pointInfoMap.get("3分钟") + "," + whole_pointInfoMap.get("5分钟");
		...
		PostData(
				...
				function(result) {
					...
					//获取所有的车辆
					setCartoMap(fromDate,toDate,strPointId,pointIdShiftMap);
					
					map.RemoveMarkByType("135");
					for(var i=0;i<allPointinfo.length;i++){
						var item=new Object();
	...
	function setCartoMap(fromDate,toDate,strPointId,pointIdShiftMap){
		//获取所有的车辆	
		var carNoMap=new Map();
		var strCarNo="";
		var strConditions1 = "pointInfoId,int,in,"+strPointId +";workDt,string,>=,"+fromDate+";workDt,string,<=,"+toDate+";DeviceTypeName,string,=,车号";
		PostData("point/pointDevice/filter",createRequest(0, 10000, "id", strConditions1),function(result1) {
			if(result1.data.length>0){
				var deviceInfos= result1.data;
				var carNoStr="";
				for(var i=0;i<deviceInfos.length;i++){								
					var carNo=deviceInfos[i].value;
				...
				if(carNoStr.length>0){
					carNoStr=carNoStr.substr(0,carNoStr.length-1);
				}
				strCarNo=carNoStr;
			}
		},false);
		if(strCarNo.length>0){
			strConditions1 = "name,string,in,"+strCarNo;
			PostData("device/deviceInfo/filter",createRequest(0, 10000, "id", strConditions1),function(result1){
				if(result1.data.length>0){
					var allCar=result1.data;
					map.RemoveMarkByType("135car");
					for(var i=0;i<allCar.length;i++){
						var item=new Object();
						item.lng=allCar[i].longitude;
						item.lat=allCar[i].latitude;
						item.name=allCar[i].name;
						item.type="135car";
						item.ID=allCar[i].id;						
						item.SX=getShowTypebyParam(pointIdShiftMap.get(carNoMap.get(allCar[i].name)),allCar[i].state);
						item.lineid=allCar[i].state;
						item.content="";
						var iconName="";
						if(allCar[i].state=="在线"){
							iconName="shift4_2_0.png";
						}
						else{
	...
	function getShowTypebyParam(pointInfoType,state){
		var showType= 0;		
		if(pointInfoType==whole_pointInfoMap.get("1分钟")){
			showType = showType | 16;
		}
		else if(pointInfoType==whole_pointInfoMap.get("3分钟")){
			showType = showType | 8;
		}
		else if(pointInfoType==whole_pointInfoMap.get("5分钟")){
			showType = showType | 4;
		}
		if(state=="在线"){
			showType = showType | 2;
	...
	function getIconFromPointInfo(pointInfo,arrngeInfosMap){
		...
		if(pointInfo.type==whole_pointInfoMap.get("1分钟")){
			iconName+="1.png"
		}
		else if(pointInfo.type==whole_pointInfoMap.get("3分钟")){
			iconName+="3.png"
		}
		else if(pointInfo.type==whole_pointInfoMap.get("5分钟")){
			iconName+="5.png"
		}
		return iconName;
	}
...
<script type="text/javascript">
	...
	function showTable(itemId,type,minutesType,title) {		
		...
		showClickTimes ++;
		var NowDate=new Date().Format("yyyy-MM-dd");
		whole_ClickMenuMap.set(identify,itemId+","+type+","+minutesType+","+title);
		...
		html2 += CreateHeadHtml(type,title,minutesType,NowDate);
		html2 += "</table></div>";
		...
		if(minutesType!=1){
			autoRowSpanByPreItem(tbody,0,3);
	...
	function changeTable(fromDate,toDate,index,isEdit){
		...
		if(item.length>=3){		
			...
			html += CreateHeadHtml(item[1],item[3],item[2],dateshow,isEdit);
			html += "</table></div>";
	...
	function CreateHeadHtml(type,title,minutesType,strDate,isEdit) {
		var html = "";
		if(minutesType == 1){
			html += "<thead>";
			html += "<tr>";
			if(isEdit == 0){
				if(type==1||type==2){
					...
					html += "<td colspan='18'>特警支队"+title+"常态化联勤武装巡逻警力部署表("+strDate+")</td>";
				}
			}else{
				if(type==1||type==2){
					html += "<td colspan='17'>特警支队常态化联勤武装巡逻警力部署表("+strDate+")</td>";
				}else if(type==3){
					html += "<td colspan='17'>特警支队"+title+"常态化联勤武装巡逻警力部署表("+strDate+")</td>";
				}
			}
			html += "</tr>"
			html += "<tr>";
			...
			if(isEdit == 0){
				if(type==1){
					html += "<td colspan='17'>各分局"+title+"巡逻安排表("+strDate+")</td>";
				}else if(type==2||type==3){
					html += "<td colspan='17'>"+title+"巡逻安排表("+strDate+")</td>";
				}
			}else{
				if(type==1){
					html += "<td colspan='16'>各分局"+title+"巡逻安排表("+strDate+")</td>";
				}else if(type==2||type==3){
					html += "<td colspan='16'>"+title+"巡逻安排表("+strDate+")</td>";
				}
			}
			html += "</tr>"
			html += "<tr>";
	...
	function createTableEditBody(listRowIdx,pointInfos,arrangeInfos,deviceInfos,substation,arrangeTimeSpan,pointInfoDeviceDict,pageId){
		...
		for(var i=0;i<listRowIdx.length;i++){
			...
			if(spanRows>0){
				for(var j=0;j<spanRows;j++){				
					...
					for(var k=0;k<arrangeIdx.length;k++){	
						...
						staffinfo += ";<span style='display: inline-block; color:white;float: right;margin-right: 5px;cursor: pointer;' onclick='delStaff(this,"+type+")'>x</span></div>";
					}
					html += "<td colspan='3' id='staffMess_"+pageId+"_"+timeSpanIdx+"' onclick='showStaffMess("+itemPoint.id+","+pageId+","+itemPoint.stationId+",\""+itemPoint.stationName+"\","+timeSpanIdx+","+type+")'>" + staffinfo + "</td>";
					...
					if(type==1){
						...
						for(var m=0;m<pointInfoDeviceDict.length;m++){
							...
							if(itemIdx[m+1].length>0){
								//车台类型时，增加弹出框
								if(m==0){
									html += "<td id='tdCarNo_"+pageId+"_"+timeSpanIdx+"' ><input id='deviceMess_"+pageId+"_"+deviceInfos[Number(itemIdx[m+1])].deviceTypeCode+"_"+timeSpanIdx+"' type='text' onclick='showCarOptions("+itemPoint.id+","+pageId+","+itemPoint.stationId+","+timeSpanIdx+")' autocomplete='off' class='layui-input input-border' value=" + deviceInfos[Number(itemIdx[m+1])].value+ "></td>";
								}
								else{
				...
				var itemPoint=pointInfos[i];
				var ItemCommMap=new Map();
				for(var k=0;k<itemPoint.pointInfoDevices.length;k++){
					var itemDevices=itemPoint.pointInfoDevices[k];
				...
				html += "<td colspan='3' id='staffMess_"+pageId+"_"+timeSpanIdx+"' onclick='showStaffMess("+itemPoint.id+","+pageId+","+itemPoint.stationId+",\""+itemPoint.stationName+"\","+timeSpanIdx+","+type+")'></td>";
				var strDeviceTypeCode = "";
				if(type==1){
					html += "<td>" + itemPoint.stationName + "</td>";					
					for(var m=0;m<pointInfoDeviceDict.length;m++){
						...
						var value="";
						if(m==1){
							value=ItemCommMap.get("组别");
						}else if(m==2){
							value=ItemCommMap.get("代码");
						}
						else if(m==3){
							value=ItemCommMap.get("电台值守组");
						}
						if(value==null) value="";
						if(m==0){
							html += "<td id='tdCarNo_"+pageId+"_"+timeSpanIdx+"'><input id='deviceMess_"+pageId+"_"+pointInfoDeviceDict[m].code+"_"+timeSpanIdx+"' type='text' autocomplete='off' class='layui-input input-border' value='"+ value+"' onclick='showCarOptions("+itemPoint.id+","+pageId+","+itemPoint.stationId+","+timeSpanIdx+")'></td>";
						}else{
							html += "<td><input id='deviceMess_"+pageId+"_"+pointInfoDeviceDict[m].code+"_"+timeSpanIdx+"' type='text' autocomplete='off' class='layui-input input-border' value='"+ value+"'></td>";
						}
			...
			html += createAddTr(temp,pageId,itemPoint,type);
		}		
		//html += "</tbody></table></div>";
		html += "</tbody>";
	...
	// 编辑删除人员
	function delStaff(obj,type) {
		var content = $(obj).parent().text().replace(";x","");
		var item = content.split(",");
		var inputId="";
		if(type!=1){
			if(item[4]==1){
				inputId=$(obj).parent().parent().next().next().next().next().children().attr("id");
			}else{
				inputId=$(obj).parent().parent().next().next().next().next().next().next().children().attr("id");
			}
		}else{
			if(item[4]==1){
				inputId=$(obj).parent().parent().next().next().next().next().next().next().next().next().children().attr("id");
			}
		}
	...
	// 创建隐藏的新增tr
	function createAddTr(temp,pageId,itemPoint,type) {
		var html = "";
		var teamtemp=""
		var codetemp="";
		var rediotemp="";
		...
		teamtemp=ItemCommMap.get("组别");
		codetemp=ItemCommMap.get("代码");
		rediotemp=ItemCommMap.get("电台值守组");
		
		if(teamtemp==null) teamtemp="";
		if(codetemp==null) codetemp="";
		if(rediotemp==null) rediotemp="";
		var timeSpanIdx;
		if(temp=="3分钟反应圈"||temp=="5分钟反应圈"){
			...
			html += "<td colspan='3' id='staffMess_"+pageId+"_"+itemPoint.id+"-' onclick='showStaffMess("+itemPoint.id+","+pageId+","+itemPoint.stationId+",\""+itemPoint.stationName+"\","+timeSpanIdx+","+type+")'></td>";
			html += "<td id='tdnCar_"+pageId+"_"+itemPoint.id+"'><input id='nCar_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off' class='layui-input input-border' onclick='showCarOptions("+itemPoint.id+","+pageId+","+itemPoint.stationId+","+timeSpanIdx+")'></td>";
			html += "<td><input id='cCode_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off' class='layui-input input-border' value='"+ codetemp+"'></td>";
			html += "<td><input id='cRidioGroup_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off'	class='layui-input input-border' value='"+ rediotemp+"'></td>";
			...
			html += "<td>"+itemPoint.stationName+"</td>";
			html += "<td id='tdnCar_"+pageId+"_"+itemPoint.id+"'><input id='nCar_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off'	class='layui-input input-border' onclick='showCarOptions("+itemPoint.id+","+pageId+","+itemPoint.stationId+","+timeSpanIdx+")'></td>";
			html += "<td><input id='cGroup_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off'	class='layui-input input-border'value='"+ teamtemp+"'></td>";
			html += "<td><input id='cCode_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off'	class='layui-input input-border' value='"+ codetemp+"'></td>";
			html += "<td><input id='cRidioGroup_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off'	class='layui-input input-border' value='"+ rediotemp+"'></td>";
			html += "<td><input id='aName_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off'	class='layui-input input-border'></td>";
			html += "<td><input id='aPhone_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off'	class='layui-input input-border'></td>";
			html += "<td><input id='nSpecial_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off'	class='layui-input input-border'></td>";
			html += "<td><input id='nArmed_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off'	class='layui-input input-border'></td>";
			html += "<td><input id='eLongGun_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off'	class='layui-input input-border'></td>";
			html += "<td><input id='eHandGun_"+pageId+"_"+itemPoint.id+"' type='text' autocomplete='off'	class='layui-input input-border'></td>";
			html += "<td><div style='margin-top: 5px;'><button class='layui-btn layui-btn-warm layui-btn-small' onclick='saveShiftAndDeviceMess(\""+temp+"\","+pageId+","+itemPoint.id+","+itemPoint.stationId+")'>保存</button></div>";
	...
	function modifyStaffAndDeviceMess(id,pageId,timeSpanIdx,strDeviceTypeCode,type,identify) {
		if(type=="3分钟反应圈"||type=="5分钟反应圈"){
			shiftType=whole_shiftTypeIdMap.get(type.replace("反应圈",""));
		}else{
			shiftType=whole_shiftTypeIdMap.get("1分钟");
		}
	...
	// 单位列表框
	function showStaffMess(id,pageId,stationId,stationName,tdId,type){
		...
		if(type!=1){
			...
			document.getElementById('staffMess').removeAttribute("xm-select-create");
		}
		document.getElementById("ensure").setAttribute("onclick","addStaffMess("+id+","+pageId+","+tdId+","+type+")");
	...
	function saveShiftAndDeviceMess(temp,pageId,pointId,stationId) {
		...
		var nSpecial;
		if(temp=="3分钟反应圈"||temp=="5分钟反应圈"){
			shiftType=whole_shiftTypeIdMap.get(temp.replace("反应圈",""));
			...
			shiftType=whole_shiftTypeIdMap.get("1分钟");
			cGroup = document.getElementById("cGroup_"+pageId+"_"+pointId).value;// 通信组别
			aName = document.getElementById("aName_"+pageId+"_"+pointId).value;// 武装联络人姓名
			aPhone = document.getElementById("aPhone_"+pageId+"_"+pointId).value;// 武装联络人电话
			nArmed = document.getElementById("nArmed_"+pageId+"_"+pointId).value;// 武警人数
			nSpecial = document.getElementById("nSpecial_"+pageId+"_"+pointId).value;// 特警人数
		}
		var searchBegin = document.getElementById("searchBegin_"+pageId).value;// 第一个搜索时间
		...
		if(addTimeSpan.split(" - ").length==2){
 		...
 		var nCar = document.getElementById("nCar_"+pageId+"_"+pointId).value;// 通信车号
		var cCode = document.getElementById("cCode_"+pageId+"_"+pointId).value;// 通信代码
		var cRidioGroup = document.getElementById("cRidioGroup_"+pageId+"_"+pointId).value;// 通信电台值守组
		var nArmed = document.getElementById("nArmed_"+pageId+"_"+pointId).value;// 武警人数
		var eLongGun = document.getElementById("eLongGun_"+pageId+"_"+pointId).value;// 长枪装备
		var eHandGun = document.getElementById("eHandGun_"+pageId+"_"+pointId).value;// 短枪装备
		var devicesInfoMap = new Map();
		var devicesInfoArr = ["通信,车号;"+nCar+"","通信,组别;"+cGroup+"","通信,代码;"+cCode+"","通信,电台值守组;"+cRidioGroup
							+"","武警联络人,姓名;"+aName+"","武警联络人,电话;"+aPhone+"","执勤人数,民警;"+nPeople+"","执勤人数,武警;"+nArmed
							+"","执勤人数,特警;"+nSpecial+"","执勤人数,协警;"+nAuxiliary+"","携带装备,长枪;"+eLongGun+"","携带装备,短枪;"+eHandGun+""];
		...
		for(var i=0;i<devicesInfoArr.length;i++){
			...
			if(temp=="3分钟反应圈"||temp=="5分钟反应圈"){
				if(item[0]=="武警联络人,姓名"||item[0]=="武警联络人,电话"||item[0]=="通信,组别"||item[0]=="执勤人数,特警"){
					continue;
				}
			}else{
				if(item[0]=="执勤人数,民警"||item[0]=="执勤人数,协警"){
					continue;
				}
			}
	...
	// 将选择的警员加入td
	function addStaffMess(id,pageId,tdId,type) {
		var nAuxiliary;
		var nPeople;
		var nSpecial;
		...
		if(type!=1){
			...
			nSpecial = document.getElementById("nSpecial_"+pageId+"_"+id);// 特警人数
		}
		if(typeof(tdId)!="undefined"){
			...
			if(type!=1){
				...
				nSpecial = document.getElementById("deviceMess_"+pageId+"_"+whole_deviceDictMap.get("执勤人数,特警")+"_"+tdId);
			}
		}
		var divsMap = new Map();
		...
		for(var i=0;i<selectVal.length;i++){
			if(strStaff.indexOf(selectVal[i].value)==-1){
				...
				if(selectVal[i].value.split(",").length!=5){
					...
					if(nSpecial!=null&&nSpecial!=""&&selectVal[i].value.split(",")[4]==1){
						nSpecial.value++;
					}
				}
				strStaff += "<span style='display: inline-block; color:white;float: right;margin-right: 5px;cursor: pointer;' onclick='delStaff(this,"+type+")'>x</span></div>";
			}else{
...
<script>
	...
	// 车台选项显示
	function showCarOptions(id,pageId,stationId,tdId){	
		createCarOption(stationId);
		var carSelection = document.getElementById("carOptionMess");
		carSelection.removeAttribute("multiple");
		var staffId = document.getElementById("tdnCar_"+pageId+"_" + id);
		if(typeof tdId !="undefined"){
			staffId = document.getElementById("tdCarNo_"+pageId+"_"+ tdId);
		}
		...
		document.getElementById("carDeviceEnsure").setAttribute("onclick","addDeviceMess("+id+","+pageId+","+tdId+")");
		$("body").bind("mousedown", onBodyMouseDown);
	}
	
	// 创建车台option
	function createCarOption(stationId) {
		PostData("device/deviceInfo/filter", createRequest(0, 1000, "id","stationId,string,=,"+stationId+";"+"type,string,=,巡逻车台"),function(result) {
			var data = result.data;
			$("#carOptionMess").empty();
			var html = "";
			for(var i=0;i<data.length;i++){
			...
			$("#carOptionMess").append(html);
			formSelects.render();
		})
	}
	
	// 将选择的设备加入td
	function addDeviceMess(id,pageId,tdId) {
		var tdContent = document.getElementById("nCar_"+pageId+"_" + id);
		if(typeof tdId !="undefined"){
			tdContent = document.getElementById("deviceMess_"+pageId+"_"+"0101" + "_"+ tdId);
		}
		
		var oldvalue=tdContent.value;
		var selectVal = formSelects.value('carOptionStaff');
		for(var i=0;i<selectVal.length;i++){
			if(oldvalue.length==0){
				oldvalue=selectVal[i].name;
			}
			else{
				oldvalue+=","+selectVal[i].name;
			}
		}
		tdContent.value=oldvalue;
		
		$('#carOptionDiv').fadeOut("fast");
		document.getElementById("carDeviceEnsure").removeAttribute("onclick");
		formSelects.render("carOptionStaff");
		$("body").unbind("mousedown", onBodyMouseDown);
	}
</script>
