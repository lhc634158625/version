...
var DaySaleReport = function () {
    ...
    this.Initial = function (index) {
       ...
       // _self.tableTmp = document.getElementById('tmpSaleData').innerHTML;
        _self.InitData();
        _self.InitEvent();
    }
    ...
    this.InitData = function () {
        ...
        _self.myChart = echarts.init(document.getElementById('divDayReportChart'));
...
;
        $("canvas").addClass("IgnoreDrag");
    }
    this.InitEvent = function () {
        $("#dayReportTitle span").click(function (e) {
            _self.option = $(this).attr("rel");
            $("#dayReportTitle span").removeClass("selected");
            $(this).addClass("selected");
            _self.DrawChart();
        })
        $("#bankChatOption li").click(function (e) {
            _self.timeOption = $(this).attr("rel");
            $("#bankChatOption li").removeClass("selected");
            $(this).addClass("selected");
            _self.RetrieveData();
        })
    }
    this.GetFromDt = function () {
        var now = new Date();
        if (_self.timeOption == "YearMonth" || _self.timeOption == "YearSeason") {
            return _self.dateTime.FormatDate(_self.dateTime.AddDate(now, 'MONTH', -12), "yyyy-MM-01")
        }
        if (_self.timeOption == "2YearMonth") {
            return _self.dateTime.FormatDate(_self.dateTime.AddDate(now, 'YEAR', -1), "yyyy-01-01")
        }
        if (_self.timeOption == "MonthWeek") {
    ...
    this.RetrieveData = function () {
        ...
        param.filter = filter;
        if (_self.timeOption == "YearMonth") {
            param.reportType = "month";
        }
        if (_self.timeOption == "YearSeason") {
            param.reportType = "season";
        }
        if (_self.timeOption == "2YearSeason") {
            param.reportType = "season";
        }
        if (_self.timeOption == "MonthWeek") {
            param.reportType = "week";
        }
        if (_self.timeOption == "Year") {
            param.reportType = "year";
        }
        PostData("GetBankReport2", param, function (result) {
    ...
    this.DrawChart = function () {
        ...
        for (var i = 0; i < reportDatas.length; i++) {
            ...
            if (_self.option == "Count") {
                data1.push(reportDatas[i].Count);
                data2.push(reportDatas[i].Count2);
            }
            else {
                data1.push(reportDatas[i].Amount);
                data2.push(reportDatas[i].Amount2);
            }
        }

        _self.chartOption.xAxis[0].data = xdata;
        _self.chartOption.series[0].data = data1;
        _self.chartOption.series[1].data = data2;
        //options.xAxis[0].data = result.category;  
        _self.myChart.hideLoading();
        _self.myChart.setOption(_self.chartOption);
